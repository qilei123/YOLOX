...
class MosaicDetectionP(Dataset):
    """Detection dataset wrapper that performs mixup for normal dataset."""

    def __init__(
        self, dataset, img_size, possibility = 1.0, mosaic=True, preproc=None,
        degrees=10.0, translate=0.1, scale=(0.5, 1.5), mscale=(0.5, 1.5),
        shear=2.0, perspective=0.0, enable_mixup=True, *args
    ):
        """
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random()<self.possibility:
            ...
            #only show the main image, sometimes will the groundtruth is not right
            '''
            mosaic_img, mosaic_labels = random_perspective(
                mosaic_img,
                mosaic_labels,
